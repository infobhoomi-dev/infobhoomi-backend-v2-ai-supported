...
class Lnd_Admin_Info_View(ListCreateAPIView):
    ...
    def get(self, request, su_id):
        try:
            ...
            response_data["dsd"] = gnd_data["dsd"] if gnd_data else None
            response_data["dist"] = gnd_data["dist"] if gnd_data else None
            response_data["pd"] = gnd_data["pd"] if gnd_data else None
            response_data["eletorate"] = elect_data["eletorate"] if elect_data else None
...
class Lnd_Overview_View(ListCreateAPIView):
    ...
    def get(self, request, su_id):
        try:
            ...
            # Always return new LADM spatial fields regardless of role permissions
            combined_data["perimeter"] = float(land_unit.perimeter) if land_unit and land_unit.perimeter is not None else None
            combined_data["boundary_type"] = land_unit.boundary_type if land_unit else None
            combined_data["crs"] = land_unit.crs if land_unit else None
...
class Lnd_Overview_Update_View(APIView):
    ...
    def patch(self, request, su_id):
        try:
            ...
            # Step 7: Always save spatial fields to la_ls_land_unit (LADM-compliant)
            lu_spatial_fields = []
            for field in ('area', 'perimeter', 'dimension_2d_3d', 'boundary_type', 'crs'):
                if field in request.data and request.data[field] not in ('', None):
                    setattr(land_unit, field, request.data[field])
                    lu_spatial_fields.append(field)
            if lu_spatial_fields:
                land_unit.save(update_fields=lu_spatial_fields)
...
class Bld_Admin_Info_View(ListCreateAPIView):
    ...
    def get(self, request, su_id):
        try:
            ...
            # Always return new physical building fields (bypass permission gate)
            combined_data["registration_date"]  = build_unit.registration_date  if build_unit else None
            combined_data["construction_year"]  = build_unit.construction_year  if build_unit else None
            combined_data["structure_type"]     = build_unit.structure_type     if build_unit else None
            combined_data["condition"]          = build_unit.condition          if build_unit else None
...
class Bld_Admin_Info_Update_View(APIView):
    ...
    def patch(self, request, su_id):
        try:
            ...
            # 4. Update LA_LS_Build_Unit_Model
            build_fields = {"access_road", "bld_name", "postal_ad_build", "house_hold_no", "bld_property_type", "no_floors",
                            "registration_date", "construction_year", "structure_type", "condition"}
            build_update_data = {k: v for k, v in filtered_data.items() if k in build_fields}
            # bld_name is the API alias for the model field building_name
            if "bld_name" in build_update_data:
                build_update_data["building_name"] = build_update_data.pop("bld_name")
            build_unit = LA_LS_Build_Unit_Model.objects.filter(su_id=su_id).first()
...
class Bld_Overview_Update_View(APIView):
    ...
    def patch(self, request, su_id):
        try:
            ...
            build_unit = LA_LS_Build_Unit_Model.objects.filter(su_id=su_id).first()
            if not build_unit:
                return Response({"error": "Building data not found for the given su_id."}, status=status.HTTP_404_NOT_FOUND)
            ...
            # Step 6: Apply update and log changes
            original_data = build_unit.__dict__.copy()
            serializer = LA_LS_Build_Unit_Serializer(build_unit, data=update_data, partial=True)
            if serializer.is_valid():
                serializer.save()
                self.log_changes(user_id, category, su_id, original_data, serializer.validated_data)
                return Response({"detail": "Data updated successfully."}, status=status.HTTP_200_OK)
...
class Bld_Utility_Network_Info_View(APIView):
    ...
    def get(self, request, su_id):
        try:
            ...
            # Step 3: Check allowed view fields
            viewable_fields = [
                field for field, perm_id in FIELD_PERMISSION_MAP.items()
                if Role_Permission_Model.objects.filter(
                    role_id=role_id,
            ...
            if not data:
                return Response({"detail": "No data found for the given su_id."}, status=status.HTTP_404_NOT_FOUND)

            serializer = Bld_Utinet_info_Serializer(data)
            filtered_data = {
                key: value for key, value in serializer.data.items() if key in viewable_fields
            }
...
class Bld_Utility_Network_Info_Update_View(APIView):
    ...
    def patch(self, request, su_id):
        try:
            ...
            # Step 4: Filter incoming data
            update_data = {field: request.data[field] for field in editable_fields if field in request.data}
            if not update_data:
                return Response({"error": "No editable fields in request."}, status=403)
...
class Tax_Assessment_Update_View(APIView):
    ...
    def patch(self, request, su_id):
        try:
            ...
            # Step 5: Update Assessment_Model
            ass_data = Assessment_Model.objects.filter(su_id=su_id).first()
            if not ass_data:
                return Response({"error": "No assessment data found."}, status=404)
            ...
            # Step 6: Update Tax_Info_Model if applicable
...
class RRR_Remove_Document_View(APIView):
    ...
    def delete(self, request, doc_link_id):
        try:
            doc_link = LA_RRR_Document_Model.objects.select_related('admin_source').get(id=doc_link_id)
